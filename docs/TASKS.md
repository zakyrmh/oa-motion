...
| **Dokumen Referensi (PRD)** | `docs/PRD.md` (v1.0 — MVP Release) |
...
Dokumen ini memecah kebutuhan produk dari `PRD.md` menjadi unit-unit tugas teknis terukur dengan target penyelesaian akhir **15 September 2026**. Dokumen ini menjadi acuan kerja operasional bagi seluruh anggota Tim SPEKTRA dan AI Coding Agent guna memastikan kejelasan ruang lingkup (*scope*), pembagian tugas (*responsibility*), dependensi antarmodul, serta definisi selesai (*Definition of Done*).
...
| **T-009** | Audio Controller `ttsEngine.ts` | Web Speech API Bahasa Indonesia (`id-ID`) dengan mekanisme anti-spam *debounce* dan *chime synthesizer*. | P0 | T-008 | 🔴 | Zaky Ramadhan |
...
| ID | Tugas | Deskripsi | Prioritas | Dependency | Status | PIC |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **T-010** | Implementasi Layar `Tracking.tsx` | Hero screen latihan: Canvas skeleton overlay, badge sudut real-time, dan status banner zona. | P0 | T-005, T-008, T-009 | 🔴 | Zaky Ramadhan |
...
| **T-012** | Implementasi Layar `Summary.tsx` | Dashboard pasca-latihan: 4 kartu metrik (Durasi, RoM, Warning, Kepatuhan). | P0 | T-011 | 🔴 | Zahwa Rahmadhania |
| **T-013** | Visualisasi Grafik RoM (Chart.js) | Render grafik garis tren sudut fleksi per repetisi dengan garis batas ambang zona. | P1 | T-012 | 🔴 | Zahwa Rahmadhania |
| **T-014** | Generator PDF Telerehabilitasi | Fitur unduh rekapitulasi data telerehabilitasi terstruktur untuk dibagikan ke fisioterapis. | P1 | T-012 | 🔴 | Zaky Ramadhan |
...
- **Kriteria Selesai (Acceptance Criteria):**
  - [ ] Menghasilkan output angka float sudut antara $0^\circ$ (kaki lurus) sampai $140^\circ$ (tekukan maksimal).
  - [ ] Tidak menghasilkan nilai `NaN` saat sendi sejajar atau titik koordinat bernilai 0.
...
- **Kriteria Selesai (Acceptance Criteria):**
  - [ ] Visual UI 100% identik dengan hasil desain Google Stitch.
  - [ ] Suara peringatan otomatis keluar tepat waktu tanpa delay terasa ($<100\text{ ms}$).
  - [ ] Tombol merah "HENTIKAN LATIHAN" langsung mematikan stream kamera dan berpindah ke `/summary`.
